...
BulkinLevel::BulkinLevel(std::string path, uint32_t wallTexture, uint32_t floorTexture, uint32_t ceilingTexture, size_t maxHeight) : wallTexture(wallTexture), floorTexture(floorTexture), ceilingTexture(ceilingTexture), maxHeight(maxHeight) {
  tmx::Map map;
  if (map.load(path)) {
